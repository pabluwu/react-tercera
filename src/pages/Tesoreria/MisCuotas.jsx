      ...
      <div className="container mx-auto px-4 py-6 md:py-8 max-w-5xl">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-8 gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-800 dark:text-white tracking-tight">Mis Cuotas</h2>
              <p className="text-slate-500 dark:text-slate-400 font-medium text-xs sm:text-sm md:text-base">Historial de pagos y pendientes</p>
            </div>
          </div>
        </div>
        ...
        {!ownId && (
          <div className="flex items-center gap-3 p-4 sm:p-5 bg-red-50 dark:!bg-red-900/20 text-red-700 dark:text-red-400 rounded-2xl sm:rounded-3xl border border-red-100 dark:border-red-900/30 mb-8">
            ...
            <p className="font-bold text-sm sm:text-base">No se encontró la información de tu usuario.</p>
          </div>
        )}
        ...
        {!isLoading && !hasError && years.length === 0 && (
          <div className="flex flex-col items-center justify-center p-10 md:p-16 !bg-white dark:!bg-slate-800 rounded-3xl md:rounded-[2.5rem] border border-slate-100 dark:border-slate-700 text-center">
            ...
            <p className="text-slate-500 dark:text-slate-400 max-w-sm text-sm md:text-base">No se encontraron registros de cuotas para tu usuario en el sistema.</p>
          </div>
        )}
      </div>
    </Layout>
